...
//		View v = (View)ll;
		...
		/*try {
			...
			NodeList nodeLst = doc.getElementsByTagName("Button");
			Log.i("MyLog" , "-----size------ = " + nodeLst.getLength());
			for (int i = 0; i < nodeLst.getLength(); i++) {
				Log.i("MyLog" , nodeLst.item(i).toString());
				...
				list_of_heights.add(getRelativeTop((Button) nodeLst.item(i))
						+ ((Button) nodeLst.item(i)).getHeight());
				list_of_buttons.add((Button) nodeLst.item(i));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
*/
		/*for (int i = 0; i < ll.getChildCount(); i++) {
			...
			// Log.i("Mylog", "LinearLayout" + view.toString() + "num " + i);
			for (int j = 0; j < ((LinearLayout) view).getChildCount(); j++) {
				View v = ((LinearLayout) view).getChildAt(j);

				// int[] location = new int[2];

				if (v instanceof Button) {
					Log.i("Mylog", "left position = " + getRelativeLeft(v)
							+ " and top = " + getRelativeTop(v));
					// rb.put(new Rect(v.getLeft(), v.getTop(), v.getRight(), v
					// .getBottom()), (Button) v);
					// v.getLocationOnScreen(location);
					// Log.i("MyLog", "location of "
					// + ((Button) v).getText().toString() + "is: "
					...
					// Rect rect = new Rect();
					// ((Button)v).getLocalVisibleRect(rect);
					// Log.i("MyLog", "location of "
					// + ((Button) v).getText().toString() + "is: "
					// +rect.left + " , "+rect.right + " , "+rect.bottom +
					// " , "+rect.top + "and width = " + rect.width() +
					// "and height = " + rect.height());

					// Rect newRect = new Rect(((Button) v).getLeft(),
					// ((Button) v).getTop(), ((Button) v).getRight(),
					// ((Button) v).getBottom());
...
*/		return;
	...
	/*private View getView(float x, float y) {
		for (int i = 0; i < list_of_buttons.size(); i++) {
			...
			if (x >= list_of_x_coords.get(i)
					&& x <= list_of_x_coords.get(i) + list_of_widths.get(i)
					&& y >= list_of_y_coords.get(i)
					&& y <= list_of_y_coords.get(i) + list_of_heights.get(i)) {

				return list_of_buttons.get(i);
			}
		}
		return null;
	}*/

	/*private void buttonsPicker(ViewGroup v, Document doc) {
		XPath xpath = XPathFactory.newInstance().newXPath();
		XPathExpression expr = null;
		try {
			expr = xpath.compile("//Button");
