...
public class VisionApplication extends Application {
	...
	color_to_string.append(R.color.LIGHT_PURPLE,
		Color.parseColor("#A901DB"));
    }
	...
	for (final View v : vs)
		...
		if (textColor == R.color.WHITE)
		    ((ImageView) v).setColorFilter(
			    color_to_string.get(backgroundColor), Mode.LIGHTEN);
		else
		    ((ImageView) v).setColorFilter(
			    color_to_string.get(textColor), Mode.DARKEN);
		((TalkingImageButton) v).setBackgroundColor(color_to_string
			.get(backgroundColor));
	    }
    }
	...
	if (v instanceof TalkingImageButton) {
		...
		((ImageView) v)
			.setColorFilter(
				color_to_string.get(R.color.LIGHT_PURPLE),
				Mode.LIGHTEN);
		((TalkingImageButton) v).setBackgroundColor(color_to_string
			.get(R.color.LIGHT_PURPLE));
	    } else
		((ImageView) v).setColorFilter(
			color_to_string.get(R.color.LIGHT_PURPLE), Mode.DARKEN);
	} else if (v instanceof TextView)
	    ((TextView) v).setBackgroundColor(color_to_string
		    .get(R.color.LIGHT_PURPLE));
    }
	...
	if (v instanceof TalkingImageButton) {
		...
		((ImageView) v).setColorFilter(color_to_string.get(textColor),
			Mode.DARKEN);
	} else if (v instanceof TextView) {
