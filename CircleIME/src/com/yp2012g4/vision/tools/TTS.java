...
public class TTS implements OnInitListener {
    ...
    private int _qm;
    private Locale _language = Locale.US;
	...
	if (staticTTS == null || !TTS.isRunning()
		|| !_engine.equals(staticTTS._tts.getDefaultEngine())) {
	    // I know this seems overly complex, but it is a workaround aroud
	    // the fact
     ...
     * it detects it's a number or something, but this is TTS engine dependent,
    ...
    /**
     * Checks whether or not the language is supported by the current TTS
     * engine.
     * 
     * @param l
     * @return
	...
	return res != TextToSpeech.LANG_MISSING_DATA
