...
public class DialScreen extends VisionActivity {
	...
	public void onActionUp(View v) {
		...
		// undo
			...
			if (dialed_number.length() == MAX_LENGTH
					|| v.getId() == R.id.button_delete) {
				dialed_number = dialed_number.substring(0,
						Math.max(0, dialed_number.length() - 1));
				read_number = read_number.substring(0,
						Math.max(0, read_number.length() - 2));
				flag = 1;
			}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		init(0, getString(R.string.dial_screen),
				getString(R.string.dial_screen));
